...
public abstract class HorizontalTextile(int width, int height) : TextileBase(width, height, new uint[width.GetArraySize() * height])
{
    ...
    internal override TextileIndex ToIndex(int ConsecutiveIndex, int NonConsecutiveIndex) => new(ConsecutiveIndex, NonConsecutiveIndex);
